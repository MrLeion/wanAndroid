package tzl.com.framework.share.encode;
...
public class BitmapCompressor implements IBitmapCompressor {
    ...
    private int bmpQuality = 2*100 * 1024;    //compress bitmap size -> 64k
    ...
    private byte[] compressBitmap2ByteArray(Bitmap bmp) {
        ...
        float zoom = (float) Math.sqrt(2*100 * 1024 / (float) output.toByteArray().length); //获取缩放比例
